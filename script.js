...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener('scroll', function() {
        ...
        // Brand parallax effect
        const brandContent = document.querySelector('.brand-content');
        if (brandContent) {
            const brandSpeed = 0.3;
            const brandOffset = scrolled * brandSpeed;
            brandContent.style.transform = `translateY(${brandOffset}px)`;
        }
        
        // Profile image parallax
        const profileImage = document.querySelector('.profile-image');
        if (profileImage) {
            const profileSpeed = 0.2;
            const profileOffset = scrolled * profileSpeed;
            profileImage.style.transform = `translateY(${profileOffset}px)`;
        }
    });
