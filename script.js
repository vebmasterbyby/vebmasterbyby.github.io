    ...
    if (heroTitle) {
        ...
        const typeWriter = () => {
            if (charIndex < textContent.length) {
                ...
                let currentText = textContent.substring(0, charIndex + 1);
                
                // Find where the gradient text should be applied
                const gradientText = 'современные';
                const beforeGradient = 'Создаем ';
                const afterGradient = ' веб-сайты';
                const authorText = 'веб студия Олега Медведева';
                
                if (currentText.includes(gradientText)) {
                    // Apply gradient to the specific word
                    const beforePart = currentText.substring(0, currentText.indexOf(gradientText));
                    ...
                    const afterPart = currentText.substring(currentText.indexOf(gradientText) + gradientText.length);
                    
                    let result = beforePart + 
                        `<span class="text-gradient">${gradientPart}</span>` + 
                        afterPart;
                    
                    // Add author text if it's been typed
                    if (currentText.includes(authorText)) {
                        result = result.replace(authorText, `<br><span class="hero-author">${authorText}</span>`);
                    }
